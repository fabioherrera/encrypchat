...
**Estado:** implementado core + Flutter shell (2026-08-12)  
...
| Node TCP | `crates/core/src/net.rs` (Tokio; fail-loud PeerOffline) |
...
| Docs | este archivo + [ffi-contract.md](ffi-contract.md) |
...
Sin relay: peer no conectado → error explícito.
...
make check-client
# Linux:
make build-client-linux   # requiere cmake/gtk
```

## Transporte

Tokio TCP + **EH01** authenticated hello (offer + E2EE proof; pin-first peers; frame `sender_token` bound to authenticated peer). libp2p request-response deferred — ver nota histórica abajo.  
...
- [ ] Demo hardware 2 dispositivos (manual en host del usuario)
- [ ] `/auditor` framing + handshake (correr al cerrar)
